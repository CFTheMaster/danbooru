  ...
  class IpAddrsControllerTest < ActionDispatch::IntegrationTest
    context "The ip addrs controller" do
      ...
      should "find by ip addr" do
        get_auth :index:_path, @user, params: {:search => {:ip_addr => "127.0.0.1"}}
        assert_response :success
      end

      should "find by user id" do
        get_auth :index:_path, @user, params: {:search => {:user_id => @user.id.to_s}}
        assert_response :success
      end

      should "find by user name" do
        get_auth :index:_path, @user, params: {:search => {:user_name => @user.name}}
        assert_response :success
      end
